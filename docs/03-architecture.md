...
- `src/ui/interaction/` 从 canonical phase、正式 registry、标签和纯 core preview 生成安全行动：主要场景启动、活动场景移动、主要搜索、显式节点物品拾取、主动撤离、医院防火门和感染护工战斗行动。拾取草稿只保存玩家选择的数量、坐标与旋转，调用正式 pickup Preview；背包网格只投影正式几何并作为 anchor 选择，不自动摆放、整理、拆分或创建实例身份。Combat 的合法行动、CTB／请求伤害／资源消耗 metadata、当前公开意图与条件式终局分支来自 player-safe core query；正式 transition plan 与安全查询共享同一行动 metadata owner。raw Combat Preview、resolution、Effects、精确敌人生命和本次隐藏风险结果不进入普通 ViewModel。显示与行动 Preview 只白名单投影正式返程、时间、超时损耗、资源与负重事实。确认后每次只发出一条 `Store.dispatch()`；撤离或战斗终局先保存 Scene Session，结算仍由下一条显式 lifecycle command 完成。
- 成功返回后的 Return Summary 与 Combat Action Result 只将 execution 前后的 canonical phase 立即投影为本地、玩家可见的展示模型；它们不是 Scene、Combat、Run Return、Hub 或 inventory 的状态 owner，关闭不发送命令。当前没有 New Run bootstrap、React Provider、完整 Application Store、UI 命令队列或终版美术。医院防火门与感染护工战斗行动已接入正式安全 Preview 与确认分派；Scene 整理、任务事件、非战斗医疗、充能、中枢操作与日结算的 React command UI 尚未接入。展示层可被未来其他渲染技术替换，只要继续读取 canonical phase 并发送正式命令。
...
- ongoing Combat 在每个玩家命令完整结算后保存一个稳定 Scene Session；胜利、逃跑或战败也只提交 Scene Session，terminal Scene 仍需后续显式生命周期命令结算。当前不支持战斗换装或完整背包整理；最小 Store 已通过统一 dispatcher 接入，React 已接入活动 Scene 的移动、主要搜索、节点拾取、主动撤离、医院防火门障碍与感染护工 Combat command UI，命令队列仍未接入。
