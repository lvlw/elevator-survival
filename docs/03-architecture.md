...
- 成功返回后的 Return Summary、Combat Action Result、Task Event Result、Scene Medical Result、Scene Battery Result、Scene Inventory Result、Hub Loadout Result、Hub Medical Result 与 Hub Survival Result 只将 execution 前后的 canonical phase 及已发生的正式结果立即投影为本地、玩家可见的展示模型；它们不是 Scene、Combat、Run Return、Hub、任务进度、医疗、充能、饱食、威胁抑制或 inventory 的状态 owner，关闭不发送命令。当前没有 New Run bootstrap、React Provider、完整 Application Store、UI 命令队列或终版美术。医院防火门、感染护工战斗行动、医院样本箱提取、Scene 非战斗医疗、Scene 非战斗电池充能、非战斗 Scene 整理、当前日中枢十二种显式 Run Loadout 整备、中枢医疗与中枢生存补给已接入正式安全 Preview 与确认分派；中枢维护和日结算的 React command UI 尚未接入。React 不拥有 inventory、装备／快捷栏资格、医疗／充能资格、生存补给效果、目标合法性、时间、流血、返程或终局规则；展示层可被未来其他渲染技术替换，只要继续读取 canonical phase 并发送正式命令。
...
- Hub mutation 不推进日期、不启动 Scene、不中途执行 Daily Settlement，也不自动结束本日；End Day 仍是独立 `run-lifecycle` 命令。最小 Store 已通过统一 dispatcher 接入；Run Loadout 的仓库、背包、装备栏与快捷栏显式整理，以及中枢医疗和中枢生存补给已接入 React。中枢医疗只读取正式 selector、参数化医疗计划和真实物品来源；中枢生存补给只读取正式饱食／威胁抑制计划，均为零场景时间、一次显式命令和一次保存。制作、拆解及中枢维护、结束本日对应 React gameplay command UI 尚未实现。
