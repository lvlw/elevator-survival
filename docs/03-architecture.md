...
- `src/ui/interaction/` 从 canonical phase、正式 registry、标签和纯 core preview 生成安全行动：主要场景启动、活动场景移动、主要搜索、显式节点物品拾取、主动撤离、医院防火门、感染护工战斗行动与医院样本箱任务事件。拾取及任务物提取草稿只保存玩家明确选择的数量、坐标与旋转，调用对应正式 Preview；背包网格只投影正式几何并作为 anchor 选择，不自动摆放、整理、拆分或创建实例身份。Combat 与任务事件的正式 transition plan 和安全查询分别共享同一 core metadata owner；任务事件普通 Preview 只显示相对污染风险、确定性资源变化及同源时间／返程后果，不读取或暴露本次 roll、精确概率、稳定样本实例身份、raw Effects 或 resulting snapshot。确认后每次只发出一条 `Store.dispatch()`；撤离、战斗或任务事件产生的 terminal Scene 先保存 Scene Session，结算仍由下一条显式 lifecycle command 完成。
- 成功返回后的 Return Summary、Combat Action Result 与 Task Event Result 只将 execution 前后的 canonical phase 立即投影为本地、玩家可见的展示模型；它们不是 Scene、Combat、Run Return、Hub、任务进度或 inventory 的状态 owner，关闭不发送命令。当前没有 New Run bootstrap、React Provider、完整 Application Store、UI 命令队列或终版美术。医院防火门、感染护工战斗行动与医院样本箱提取已接入正式安全 Preview 与确认分派；Scene 整理、非战斗医疗、充能、中枢操作与日结算的 React command UI 尚未接入。展示层可被未来其他渲染技术替换，只要继续读取 canonical phase 并发送正式命令。
...
- ongoing Combat 在每个玩家命令完整结算后保存一个稳定 Scene Session；胜利、逃跑或战败也只提交 Scene Session，terminal Scene 仍需后续显式生命周期命令结算。当前不支持战斗换装或完整背包整理；最小 Store 已通过统一 dispatcher 接入，React 已接入活动 Scene 的移动、主要搜索、节点拾取、主动撤离、医院防火门障碍、感染护工 Combat command UI 与医院样本箱任务事件，命令队列仍未接入。
