...
| 玩家生命、流血、伤口、挫伤、暴露与镇痛 | DEC-026、DEC-031、DEC-034、DEC-035 | `combat.player`、`combat.escape`、`medical.*`、`scene.postActionBleedingDamage` | `src/core/condition/`、`src/core/daily-state/`、`src/core/combat/`、`src/core/scene-exploration/` | `src/core/condition/condition.test.ts`、日级医疗状态与医院场景／战斗医疗集成测试 | 部分实现 | 已实现类型化伤口、待处理感染暴露、日级消毒剂次数、场景绷带／止痛药／消毒剂／急救包及战斗绷带／止痛药；感染进展和电梯中枢医疗尚未实现。 |
...
| Run 最小整备库存 | DEC-007、DEC-015、DEC-016、DEC-017、DEC-023 | `backpack.*`、医院物理／资源／装备／快捷栏／返回生命周期目录 | `src/core/run-loadout/` | `src/content/hospital-v0.1/run-loadout.integration.test.ts` | 已实现最小边界 | 已实现从返回快照进入整备库存、仓库↔背包、背包↔装备、背包↔快捷栏及快捷栏内部整理；任务储存区只读。迁移保持真实实例和ItemState，堆叠抽取采用确定性新实例ID，无自动整理、合并或补充。中枢医疗、维修、充电、制作、拆解、日结算、完整RunState和UI尚未实现。 |
...
| 快捷栏 | DEC-007、DEC-016、DEC-034 | `backpack.quickSlotCount`、`medical.*`、医院快捷栏资格与医疗绑定 | `src/core/quick-slot/`、`src/core/combat/`、`src/core/scene-exploration/`、医院物品内容目录 | 核心快捷栏、命令严格性和ItemState移除测试，医院场景及战斗医疗集成测试 | 已实现医疗物品使用 | 已实现实际单件实例、堆叠抽取、放回、槽间转移，及场景绷带／止痛药／消毒剂／急救包和战斗绷带／止痛药的显式具体命令、原子消耗与不自动补充；电梯中枢和UI使用尚未实现。 |
