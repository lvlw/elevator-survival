...
| 场景单级警觉 | DEC-032 | 医院防火门内容、`combat.infectedOrderly.firstActionTime` | 场景探索警觉状态与战斗首次遭遇入口 | 医院防火门与战斗时间轴测试 | 部分实现 | `unalerted/alerted` 决定独立战斗快照首次行动70/50；场景节点自动触发和写回尚未接入。 |
...
| 感染护工单敌人CTB战斗 | DEC-006、DEC-026、DEC-031、DEC-032、DEC-035、DEC-036、DEC-037 | `combat.*` | `src/core/combat/`、`src/content/hospital-v0.1/combat/` | 核心敌人目录测试、医院战斗时间轴与风险集成测试 | 部分实现 | 已完成严格战斗恢复边界、敌人循环一致性、运行时内容绑定、连续CTB Effect链，以及首次／重入快照、基础攻击、蓄力击打、防御、临时攻击、抓挠、扑咬、风险和战斗终局；逃跑、快捷医疗、场景时间写回和节点触发尚未实现。 |
