...
| 场景节点主要搜索 | DEC-010、DEC-013、DEC-023、DEC-032 | 医院搜索定义目录 | `src/core/scene-search/`、`src/content/hospital-v0.1/search/` | `src/core/scene-search/scene-search.test.ts`、医院搜索集成测试 | 部分实现 | 已实现搜索定义、独立随机物化、场景创建时预定结果和节点揭示状态；搜索命令、时间、照明消耗和拾取尚未实现。 |
