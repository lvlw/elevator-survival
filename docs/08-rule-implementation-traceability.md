...
| 场景战斗遭遇 | DEC-006、DEC-029、DEC-031、DEC-035 | 医院遭遇目录、`combat.sceneTimeConversion` | `src/core/scene-combat/`、`src/core/scene-exploration/` | 场景遭遇核心测试、医院进入／胜利／逃跑／重入／超时集成测试 | 已实现 | 已实现节点进入触发、危险持久、逃跑返回实际来源、终局一次写回及胜利解除；不是完整RunState。 |
