...
本节是后续 UI、Interaction 与 Presentation 任务的查阅入口，不创建新的 UIR、玩法规则或内容参数。它将 UIR-015 及其 Owner 后续确认的局部呈现安排集中为当前默认；较早 UIR 的原文保留为历史记录，除本节和 UIR-015 明确局部覆盖的范围外仍有效。正式玩法、玩家知识、容器、时间、风险和生命周期仍以后续已确认 DEC、Vertical Slice 与 Content Docs 为准。

### 约定、来源与实现状态

| 主题 | 当前有效约定 | 来源／局部覆盖 | 当前实现状态 |
| --- | --- | --- | --- |
| 主舞台与一屏 | Hub、房间或战斗为中央操作与视觉主体；主要决策不依赖整页滚动。详细信息可局部滚动，但不能裁掉必要控件或把文字缩小到难以阅读来伪装一屏。 | UIR-015 与 Stage-First 后续确认，细化 UIR-008 | 已接入医院一日 Shell；具体断点与尺寸仍是可替换 Presentation 细节。 |
| 节点背景与导航 | 节点图只作为有界中央房间舞台背景；急诊大厅图只映射急诊大厅，隔离走廊 plate 用于对应战斗。小地图只读展示玩家已知空间；正式可去方向与主动返程只在房间 Travel Cluster，不能形成另一套同级主入口。 | UIR-011、UIR-015、Stage-First；DEC-045 继续优先 | 已接入；地图不发送 gameplay command。 |
| 按钮与携带区 | 图标优先、短动作名；“拾取”“取出”“卸下”“充能”等名称清楚。来源、坐标、目标与后果进入安全说明，不能因短文案省略必需参数。装备／背包切换，两个快捷位常驻；真实 `6×4` footprint、两个快捷位和计重边界来自正式规则。 | UIR-015，DEC-016／DEC-017 继续优先 | 已接入；切换仅改变展示，不复制 loadout。 |
| 物品显示与菜单 | 背包和快捷栏使用图片，缺图用占位图；物品名不必常驻网格，详情必须仍可访问；数量大于 1 时显示堆叠角标。背包与快捷栏当前使用右键菜单，并提供键盘菜单键或等价入口；左键不再打开该菜单。装备、仓库和地面物操作保持就地，不将右键要求扩张为所有界面规则。 | UIR-015 的 Owner 后续呈现细化 | 已接入。 |
| Tooltip、Ghost 与较大确认层 | Tooltip／Ghost 锚定触发元素或鼠标，键盘 Focus 锚定控件并处理边缘和遮挡；不使用固定右上角浮层。真正需要的保护确认和较大详情优先居中，可双列组织变化事实并隐藏无变化项；小型 Tooltip／菜单不因此居中。 | UIR-010、UIR-015 | 已接入；具体样式不是永久规则。 |
| 搜索与结果 | 首次搜索正式成功后，当前房间自动展开结果；关闭后可重开。重开只查看当前 canonical 已揭示物品，不重搜、不补物资，也不形成第二份掉落真相；条目直接提供拾取。 | UIR-015 后续确认 | 已接入。 |
| 普通节点拾取 first-fit | 玩家明确点击拾取后，Interaction 基于最新 canonical Scene，以固定 row-major（先 y 后 x）逐格用正式 Preview 检查未旋转放置。它不移动、重排或旋转已有物品，不自动旋转待拾取物、装备、补快捷栏，或另执行整理／拆分／合并命令；多数量仍先由玩家明确输入，无合法未旋转方案再进入手动放置／旋转。正式 pickup command 仍按 DEC-017 优先补充兼容未满堆叠后处理剩余数量；因此不能把“没有空格”写成必然不能拾取。样本箱 Task Event 的新生成／取得仍显式选择位置；已经存在于节点的实体仍按普通 pickup 规则处理。 | UIR-015 Owner Review Fix Pass 2；DEC-017 规则优先 | 已接入普通 revealed node item pickup；特殊 Task Event 放置保持显式。 |
| 结果、Feed 与滚动 | 普通结果由舞台、HUD 与 Feed 表达，详细报告按需查看；搜索结果自动展开是对象交互，不等于恢复通用事务报告。保存失败单独提示，不伪装回滚。Activity Feed 是唯一 session-local Presentation owner，Hub／Room／Battle 使用同源过滤；桌面最近六条可见是 Owner 偏好，不是持久化历史规则。滚动条使用统一主题轨道／滑块并隐藏默认两端箭头的实现意图；平台差异不能伪称已跨浏览器验收，仍优先减少不必要滚动。 | UIR-013、UIR-015 后续确认 | Feed 已接入；不进入 Store、Save、Profile 或 gameplay truth。 |
| 战斗与素材 | 中央内容区切换为 Battle Stage：双方公开状态、玩家背身槽、敌人正面、相对时间轴、行动条和同源战斗日志同属舞台。无精确敌人 HP、连续百分比生命或 raw CTB。玩家背身仍是可替换占位，敌人优先复用五个公开阶段。 | UIR-012、UIR-015；DEC 继续优先 | 已接入低资产战斗舞台和同源 Feed；最终人物资产与 Art Direction 未冻结。 |
| DEV 工具 | Inspector 保持只读。独立 DEV Reset 经不可逆保护仅清当前 Run Save，随后进入正式 no-run／Setup；它不清全部缓存、Profile，亦非生产 Run Abandon。 | UIR-004、UIR-014 | 已接入 DEV composition；Production 无入口。 |

### 分级确认与明确边界

- Level 0 是 Hover／Focus、Ghost、Tooltip、Tab 等只读行为：零 dispatch、零保存、零玩法 mutation。
- Level 1 对完整普通意图一次执行；Level 2 只补数量、来源、目标、位置、旋转或方案等必要参数，不额外套通用确认。
- Level 3 保留主动返程、End Day、任务关键物丢弃、既有破坏性操作，以及正式安全 Preview 确认本命令必死且仍有不保证死亡的合法替代行动。
- `not-guaranteed` 是正式安全事实支持非死亡分支，不代表安全；`unknown` 不是安全，也不能被用来宣称所有选择必死。条件性终局不自动升级为整条命令必死。
- 保护确认绑定原命令及重要安全事实，提交前仍重建并验证；terminal Scene settlement 不因减少确认而自动执行。
- 已知欠账：绷带、急救包等部分医疗入口仍有额外参数化确认；Owner 已接受其暂不阻塞，统一简化尚未完成。

### 实现、体验与候选的状态边界

- **已实现的工程能力：** 当前表中标为“已接入”的 Shell、Feed、地图、战斗、first-fit、右键菜单、DEV Reset 仅表示当前源码已有对应 Presentation／Interaction；不将其写成新的 Core 规则。
- **Owner 体验记录：** Owner 已确认 UIR-015 与后续呈现方向；本轮文档同步不构成后续完整体验复验。浏览器补验状态须在具体任务或审查报告中单独记录。
- **已知欠账／待核查：** 部分医疗额外确认、first-fit 重复 Preview 的性能、Feed 文案深化、既有 CI 依赖审计摘要与 bundle／Actions 告警，以及浏览器补验。
- **未来候选／暂停：** 拖拽、Context-aware Preflight、正式玩家背身美术、声音重设计与完整 Audio Pack。Audio Probe 技术接入不等于听感认可；70/30 方向仍为 provisional，后续生产暂停。
...
地图只消费 DEC-045 的显式玩家已知导航查询：未知节点／路线完全省略，已知但当前受阻仍显示阻塞；当前通行资格由正式查询决定。当前地图只读，正式可执行相邻移动与主动返程在房间 Travel Cluster；它们不创建自动寻路、多段队列、瞬移、自动破障或对完整 SceneGraph 的新解释。大地图可作为辅助展示而不改变知识规则。
...
本段仅细化 UIR-015 的 Presentation 层级，不改变四级命令、正式玩家知识、物品放置、战斗或生命周期规则。探索时当前房间是中央主舞台：已有节点图只铺在该有界舞台，不扩展成整页或整个医院背景；当前节点、时间、搜索、障碍、地面物和正式相邻可去方向叠加其上。地图继续只显示玩家已知关系，但降为只读辅助查看；主要移动入口在当前房间 Travel Cluster。这一偏好局部取代上文“无需在地图之外寻找重复前往按钮”的入口安排，不产生另一份通行资格。
...
医院一日 Player-Known Mini Map 只辅助展示已知空间关系，正式移动入口仅在当前房间 Travel Cluster。搜索正式提交后当前房间自动展开搜索结果；关闭后仍可按需再次查看。普通 revealed node item pickup 允许 UI Interaction 层基于最新 canonical Scene，以未旋转姿态按先行后列（y 再 x）的 row-major 顺序逐格通过正式 Preview 验证，并选择第一个合法位置执行同一正式 pickup command；数量有多种选择时玩家先明确数量。若未旋转位置均不合法，则回到手动位置／旋转选择。UI first-fit 不移动、整理、旋转现有物品，不自动旋转待拾取物，也不额外执行整理／拆分／合并命令；正式 pickup command 仍按 DEC-017 优先补充兼容未满堆叠后处理剩余数量。样本箱 Task Event 和其他明确要求放置取舍的特殊输出仍显式选择位置；已经存在于节点的实体不因此失去普通 pickup 路径。这是交互便利，不覆盖 Inventory Core 合法性规则。
