...
- 状态：Design direction Confirmed；Implementation Pending；Final Art / UI Skin Freeze Not established。
...
- 目标：将已实现的医院规则闭环放进可自然操作的一屏游戏界面，再进行 Owner Playability Review。新方向尚未在代码实现，不能把工程验证 PASS 当作体验验收 PASS。

### 分级执行与正式命令边界

不再要求所有正式行动都执行“点击 → 通用 Modal Preview → Confirm”。高频且必需输入已经明确的常规行动，可以由图标、短标签或地图入口单击执行；需要数量、位置、来源、目标或方案的行动，在当前区域局部展开、显式选择并执行，不再额外套通用 Modal；敏感或高风险操作保留必要二次确认，并在提交前说明后果。**完整的敏感／高风险行动分类表与触发条件尚未冻结**，不得自行把每次耐久消耗、所有可能受伤的攻击或新造的生命／概率阈值归入二次确认，从而恢复每步弹框。损坏存档清理、DEV Reset 等已确认不可逆保护继续有效。
...
下方中部的行动日志与战斗日志沿用 UIR-013 的同一 session-local Presentation Feed：只由已提交的正式执行、执行前后玩家安全事实和正式结果投影；不保存 raw snapshot、Effects、隐藏随机轨迹或内部命令身份，不进入 Store／StableRunPhase／Save／Scene／Hub／Profile，不成为 Replay truth，刷新不伪造旧历史。Hover、查看、草稿选择、取消及验证失败不记作已执行行动。Feed 和共享战斗日志仍待实现。
...
仍待细化、不得升级为已确认实现：拖拽仅 Candidate／Optional enhancement，点击与目标格选择仍可用，拖拽不能绕过 Preview／校验／正式命令；敏感／高风险确认的逐行动分类与触发条件；玩家背身的实际资产、最终形象／武器变体、新舞台尺寸和敌人素材是否需局部调整。现有音频 Probe 的技术验证及接入不等于 Owner 听感认可；Owner 反馈“当前声音不太合适，暂时放下，后面再处理”，后续调整与扩包暂停，70/30 方向仍为 PROVISIONAL。本记录不静音、删除或替换现有 WAV／播放代码。
...
- 不扩展：本轮不实现 UI、新素材、完整七日、专长、Profile、制作／拆解、多世界或结局；新方向需经后续工程实现和 Owner 再试玩，当前医院一日的工程闭环不等于整体玩法体验完成。
