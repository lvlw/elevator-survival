...
export const hospitalMainSearchDefinitions = deepFreeze([
  {
    ...
    weightedItemChoice: {
      entries: [
        { grant: { definitionId: HOSPITAL_ITEM_IDS.standardBattery, quantity: 1, initialState: { kind: 'none' } }, weight: 40 },
        { grant: { definitionId: HOSPITAL_ITEM_IDS.fabric, quantity: 1, initialState: { kind: 'none' } }, weight: 30 },
        { grant: { definitionId: HOSPITAL_ITEM_IDS.electronicComponents, quantity: 1, initialState: { kind: 'none' } }, weight: 30 },
      ],
    },
  ...
  },
  {
    ...
    weightedItemChoice: {
      entries: [
        { grant: { definitionId: HOSPITAL_ITEM_IDS.disinfectant, quantity: 1, initialState: { kind: 'none' } }, weight: 35 },
        { grant: { definitionId: HOSPITAL_ITEM_IDS.painkiller, quantity: 1, initialState: { kind: 'none' } }, weight: 30 },
        { grant: { definitionId: HOSPITAL_ITEM_IDS.firstAidKit, quantity: 1, initialState: { kind: 'none' } }, weight: 20 },
        { grant: { definitionId: HOSPITAL_ITEM_IDS.infectionSuppressant, quantity: 1, initialState: { kind: 'none' } }, weight: 15 },
      ],
    },
    fixedIntelIds: [],
  },
