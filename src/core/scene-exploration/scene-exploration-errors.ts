export type SceneExplorationErrorCode =
  | 'INVALID_STATUS'
  | 'INVALID_CURRENT_NODE'
  | 'INVALID_REMAINING_TIME'
  | 'STATUS_HEALTH_CONFLICT'
  | 'SCENE_NOT_ACTIVE'
  | 'SCENE_NOT_IN_COMBAT'
  | 'INVALID_COMBAT_STATE'
  | 'EFFECT_COMBAT_MISMATCH'
  | 'PLAYER_DEAD'
  | 'SCENE_TIME_EXHAUSTED'
  | 'INVALID_SCENE_WITHDRAWAL_COMMAND'
  | 'SCENE_WITHDRAWAL_NOT_AVAILABLE'
  | 'EFFECT_WITHDRAWAL_MISMATCH'
  | 'INVALID_EDGE_ID'
  | 'INVALID_MOVE_COMMAND'
  | 'UNKNOWN_EDGE'
  | 'EDGE_NOT_ENABLED'
  | 'EDGE_NOT_CONNECTED'
  | 'EDGE_NOT_KNOWN'
  | 'CANNOT_CARRY'
  | 'ACTION_NOT_AVAILABLE'
  | 'NO_RETURN_ROUTE'
  | 'BACKPACK_CONFIG_MISMATCH'
  | 'INVALID_INPUT'
  | 'INVALID_ILLUMINATION_CHOICE'
  | 'MAIN_SEARCH_NOT_AVAILABLE'
  | 'MAIN_SEARCH_ALREADY_COMPLETED'
  | 'NODE_NOT_SEARCHED'
  | 'UNKNOWN_NODE_ITEM'
  | 'NODE_ITEM_NOT_AT_CURRENT_NODE'
  | 'INVALID_PICKUP_QUANTITY'
  | 'INVALID_EXTRACTED_INSTANCE_ID'
  | 'PARTIAL_PICKUP_NOT_ALLOWED'
  | 'DUPLICATE_DESTINATION_INSTANCE'
  | 'INVALID_BACKPACK_PLACEMENT'
  | 'ILLUMINATION_PROVIDER_NOT_EQUIPPED'
  | 'INVALID_ILLUMINATION_PROVIDER'
  | 'INSUFFICIENT_ILLUMINATION_CHARGE'
  | 'EMPTY_EFFECTS'
  | 'UNKNOWN_EFFECT'
  | 'INVALID_EFFECT_ORDER'
  | 'EFFECT_NODE_MISMATCH'
  | 'EFFECT_NAVIGATION_MISMATCH'
  | 'EFFECT_TIME_MISMATCH'
  | 'EFFECT_HEALTH_MISMATCH'
  | 'EFFECT_STATUS_MISMATCH'
  | 'EFFECT_HEALTH_RESULT_MISMATCH'
  | 'EFFECT_RESOURCE_MISMATCH'
  | 'EFFECT_SEARCH_MISMATCH'
  | 'EFFECT_PICKUP_MISMATCH'
  | 'INCOMPLETE_EFFECT_PLAN'
  | 'UNKNOWN_OBSTACLE'
  | 'UNKNOWN_OBSTACLE_OPTION'
  | 'OBSTACLE_NOT_AT_CURRENT_NODE'
  | 'OBSTACLE_ALREADY_RESOLVED'
  | 'OBSTACLE_QUALIFICATION_FAILED'
  | 'EFFECT_OBSTACLE_MISMATCH'
  | 'EFFECT_ALERT_MISMATCH'
  | 'EFFECT_CONTUSION_MISMATCH'
  | 'EFFECT_SPAWN_MISMATCH'
  | 'EFFECT_RISK_MISMATCH'
  | 'INVALID_SCENE_MEDICAL_COMMAND'
  | 'INVALID_SCENE_MEDICAL_BINDINGS'
  | 'SCENE_MEDICAL_NOT_AVAILABLE'
  | 'EFFECT_MEDICAL_MISMATCH'
  | 'INVALID_SCENE_BATTERY_COMMAND'
  | 'INVALID_SCENE_BATTERY_BINDINGS'
  | 'SCENE_BATTERY_NOT_AVAILABLE'
  | 'EFFECT_BATTERY_MISMATCH'
  | 'INVALID_SCENE_TASK_EVENT_COMMAND'
  | 'UNKNOWN_SCENE_TASK_EVENT'
  | 'UNKNOWN_SCENE_TASK_EVENT_OPTION'
  | 'SCENE_TASK_EVENT_NOT_AT_CURRENT_NODE'
  | 'SCENE_TASK_EVENT_ALREADY_COMPLETED'
  | 'SCENE_TASK_EVENT_QUALIFICATION_FAILED'
  | 'EFFECT_TASK_EVENT_MISMATCH'

export class SceneExplorationError extends Error {
  readonly code: SceneExplorationErrorCode

  constructor(code: SceneExplorationErrorCode, message: string) {
    super(message)
    this.name = 'SceneExplorationError'
    this.code = code
  }
}
