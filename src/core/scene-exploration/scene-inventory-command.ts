...
export function createSceneInventoryCommand(input: unknown): SceneInventoryCommand {
  ...
  if (kind === 'move-scene-backpack-item' && keys(candidate, ['kind','instanceId','placement']) && id(candidate.instanceId) && placement(candidate.placement, true)) return deepFreeze(candidate as SceneInventoryCommand)
  if (kind === 'split-scene-backpack-stack' && keys(candidate, ['kind','sourceInstanceId','quantity','placement']) && id(candidate.sourceInstanceId) && integer(candidate.quantity) && candidate.quantity > 0 && placement(candidate.placement, false)) return deepFreeze(candidate as SceneInventoryCommand)
  if (kind === 'merge-scene-backpack-stacks' && keys(candidate, ['kind','sourceInstanceId','targetInstanceId','quantity']) && id(candidate.sourceInstanceId) && id(candidate.targetInstanceId) && integer(candidate.quantity) && candidate.quantity > 0) return deepFreeze(candidate as SceneInventoryCommand)
  if (kind === 'scene-backpack-to-quick-slot' && keys(candidate, ['kind','instanceId','targetSlotIndex']) && id(candidate.instanceId) && integer(candidate.targetSlotIndex)) return deepFreeze(candidate as SceneInventoryCommand)
  if (kind === 'scene-quick-slot-to-backpack' && keys(candidate, ['kind','sourceSlotIndex','placement']) && integer(candidate.sourceSlotIndex) && placement(candidate.placement, false)) return deepFreeze(candidate as SceneInventoryCommand)
  if ((kind === 'drop-scene-backpack-item' || kind === 'confirm-drop-scene-quest-item') && keys(candidate, ['kind','instanceId']) && id(candidate.instanceId)) return deepFreeze(candidate as SceneInventoryCommand)
  invalid('场景整理命令结构无效')
}
...
export function applySceneInventoryEffects(snapshot: SceneExplorationSnapshot,effects: readonly SceneExplorationEffect[],dependencies: SceneExplorationDependencies){const effect=effects[0];if(!effect||effect.kind!=='scene-inventory-committed'||effects.length!==1)throw new SceneExplorationError('EFFECT_RESOURCE_MISMATCH','整理Effect无效');const expected=buildSceneInventoryTransitionPlan(snapshot,effect.command,dependencies);if(!same(effects,expected.effects))throw new SceneExplorationError('EFFECT_RESOURCE_MISMATCH','整理Effect被篡改');return expected.snapshot}
