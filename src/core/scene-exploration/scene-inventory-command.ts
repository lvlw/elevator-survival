...
export function buildSceneInventoryTransitionPlan(snapshotInput: SceneExplorationSnapshot, input: unknown, dependencies: SceneExplorationDependencies) {
  ...
  if(command.kind==='scene-quick-slot-to-backpack'){if(command.sourceSlotIndex<0||command.sourceSlotIndex>=quickSlots.slots.length)invalid('快捷栏来源无效');const moved=quickSlots.slots[command.sourceSlotIndex];if(!moved)invalid('快捷栏来源为空');backpack=addItemToBackpack(backpack,moved!,{instanceId:moved.instanceId,...command.placement},dependencies.physicalCatalog);const backpackWeight=calculateBackpackWeightSubtotal(backpack,dependencies.physicalCatalog);if(!classifyLoad(backpackWeight,dependencies.config.backpack).canCarry)throw new SceneExplorationError('CANNOT_CARRY','该操作会使背包进入无法携带状态');const slots=[...quickSlots.slots];slots[command.sourceSlotIndex]=null;quickSlots=createQuickSlotSnapshot(slots,slots.length,dependencies.physicalCatalog,dependencies.quickSlotCatalog)}
