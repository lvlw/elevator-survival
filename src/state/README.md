...
本边界不拥有游戏规则，不保存事务中间态、Effect 计划、预览或派生缓存。`command-execution/` 只在生命周期专用 handler 已完成正式规则结算并返回稳定阶段后，按显式策略复用这一存档边界；规则拒绝不写入，写入失败不会回滚已提交结果。

当前未实现 Profile 持久化、完整 RunState、Zustand／UI 编排、多个存档槽、存档历史及迁移。
