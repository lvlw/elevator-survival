...
当前是可替换的策略控制台。已接入第一批确认式 gameplay command wiring：中枢启动主要场景、活动场景移动，以及使用／不使用手电筒的主要搜索。每个动作先由正式纯 Preview 投影为安全信息，确认后才通过 `Store.dispatch()` 发送一条正式 application command。导航只显示**当前可通行相邻节点**，不是完整场景路线总览；阻挡路线和障碍需要未来正式的玩家可见导航查询。UI 仍未接入 New Run、React Provider、完整应用编排，以及拾取、整理、撤离、障碍、任务事件、医疗、充能、战斗、中枢操作或日结算命令。开发预览和检查器只在开发环境存在；检查器不提供 mutation，预览选择器只切换独立合法 Store，不发送 gameplay command，也不保存。
